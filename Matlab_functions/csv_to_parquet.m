function csv_to_parquet(options)
% A Matlab function to convert time series data from CSV files to a Parquet file.
% 
...
%   directories.
% - DateTimes: name of a column in the CSVs to parse as datetimes.
% - Index: name of a column in the CSVs to use as an index.
...
%   the file as 64-bit floats and ints. By default, assumes that the CSVs
...
%   directories.
% - CSVPattern: Regular expression pattern to match for CSV filenames.
...
HighPrecision = false;
Verbose = false;
CSVPattern = '*.csv'; % it is not sensitive to upper/lower case
...
elseif nargin==1
    ...
    if isfield(options,'CSVPattern')
        CSVPattern = options.CSVPattern;
    end
...
for i = 1:length(campaign_directories)
    ...
    for j = 1:length(experiment_directories)
        ...
        csv_listing = dir([campaign_directory '\' experiment_directory ...
                           '\' CSVPattern]);
        ...
        csv_filenames = {csv_listing(:).name};
        csv_filenames = sort(csv_filenames);
        
        % Count the original size of these files and concatenate the data
        Table = table();
        for k = 1:length(csv_filenames)
            csv_filename = [campaign_directory '\' experiment_directory ...
                            '\' csv_filenames{k}];
            
            % Get size of csv file
            f_csv = dir(csv_filename);
            original_sizes = original_sizes+f_csv.bytes;
            ...
            % it's a good file
            if ~HighPrecision
                for m = 1:length(opts.VariableTypes)
                    if strcmp(opts.VariableTypes{m},'double')
                        opts.VariableTypes{m} = 'single';
                    end
                end
            end
            
            % Set the DateTimes column
            if any(DateTimes)
                dt_index = find(strcmp(opts.VariableNames,DateTimes));
                if ~any(dt_index)
                    error([' Is datetime column ' DateTimes ' correct?']);
                end
                opts.VariableTypes{dt_index} = 'datetime';
            end
            
            % Concatenate tables
            Table = [Table; readtable(csv_filename,opts)];
        end
